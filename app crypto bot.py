...
def fetch_candles(product, days):
    """Scarica dati Coinbase Exchange 1h con validazione rigorosa."""
    end = (int(time.time()) // 3600) * 3600
    start = end - int(days) * 24 * 3600
    step = 300 * 3600  # Exchange API: max 300 candele per richiesta
    rows = []
    cur = start
    ...
    while cur < end:
        ...
        try:
            r = requests.get(
                ...
                params=params,
                timeout=20,
                headers={"User-Agent": "CryptoBotSimulator/6.0"},
            )
    ...
    # No duplicati: se presenti, il dataset non è accettato.
    ...
    if int(df["start"].iloc[0]) != expected_start:
        return None
    if int(df["start"].iloc[-1]) != expected_last:
        return None

    # Nessun buco superiore a 1 ora: per un dataset orario completo
    # ogni timestamp deve essere esattamente a 3600 secondi dal precedente.
