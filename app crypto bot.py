...
def test_parameter_variants_trend_following(data_dict, initial_capital, threshold1, acc1, threshold2, acc2, threshold3, acc3):
    """
    Trend Following Puro - senza TP/SL rigidi.
    
    Logica:
    - Entra quando: prezzo > EMA50 AND EMA50 > EMA200 (trend UP confermato)
    - Mantiene: fintanto che prezzo > EMA50
    - Esce quando: prezzo scende sotto EMA50 (trend gira)
    """
    
    # 10 varianti con parametri diversi per EMA e Volume
    variants = [
        {"name": "Trend50_200_Strict", "ema_fast": 50, "ema_slow": 200, "vol_check": True},
        {"name": "Trend30_100_Fast", "ema_fast": 30, "ema_slow": 100, "vol_check": True},
        {"name": "Trend40_150_Balanced", "ema_fast": 40, "ema_slow": 150, "vol_check": True},
        {"name": "Trend20_50_Quick", "ema_fast": 20, "ema_slow": 50, "vol_check": False},
        {"name": "Trend60_250_Conservative", "ema_fast": 60, "ema_slow": 250, "vol_check": True},
        {"name": "Trend35_120_Optimal", "ema_fast": 35, "ema_slow": 120, "vol_check": True},
        {"name": "Trend25_80_Aggressive", "ema_fast": 25, "ema_slow": 80, "vol_check": False},
        {"name": "Trend45_180_Stable", "ema_fast": 45, "ema_slow": 180, "vol_check": True},
        {"name": "Trend55_220_Premium", "ema_fast": 55, "ema_slow": 220, "vol_check": True},
        {"name": "Trend38_140_Adaptive", "ema_fast": 38, "ema_slow": 140, "vol_check": True},
    ]
    ...
    for idx, variant in enumerate(variants):
        ...
        for crypto, df in data_dict.items():
            if df is not None and len(df) > variant["ema_slow"] + 20:
        ...
        start_idx = max(variant["ema_slow"], 220)
        
        for i in range(start_idx, num_candles - 1):
            # Se non hai posizione, cerca crypto in trend UP
            if position is None:
                best_crypto = None
                ...
                for crypto, ind_df in indicators_dict.items():
                    if i < len(ind_df):
                        ...
                        # Condizioni di entrata TREND UP:
                        # 1. Prezzo sopra EMA50
                        # 2. EMA50 sopra EMA200 (trend confermato)
                        # 3. Volume > media (optional)
                        
                        fast_ema = row.get(f'ema{variant["ema_fast"]}') if f'ema{variant["ema_fast"]}' in row.index else row['close']
                        slow_ema = row.get(f'ema{variant["ema_slow"]}') if f'ema{variant["ema_slow"]}' in row.index else row['close']
                        
                        price = float(row['close'])
                        vol_check = (row['volume'] > row['vol_ma20']) if variant["vol_check"] else True
                        
                        if price > fast_ema and fast_ema > slow_ema and vol_check:
                            # Calcola forza trend
                            trend_strength = (price - fast_ema) / fast_ema
                            
                            if trend_strength > best_trend_strength:
                                best_trend_strength = trend_strength
                                best_crypto = crypto
                
                # Se trovata crypto in trend, entra
            ...
            # Se hai posizione, monitora il trend
            else:
                ...
                if i + 1 < len(ind_df):
                    ...
                    fast_ema = nxt.get(f'ema{variant["ema_fast"]}') if f'ema{variant["ema_fast"]}' in nxt.index else nxt['close']
                    price = float(nxt.close)
                    
                    # Criterio di uscita: prezzo scende sotto EMA fast (trend gira)
                    exit_price = None
                    reason = None
    ...
    # Definisci 10 varianti di parametri - AUMENTATE MIN_SCORE (solo segnali forti)
    variants = [
        {"name": "Conservative", "tp": 0.02, "sl": 0.01, "min_score": 80, "exit_score": 60},
        {"name": "Aggressive", "tp": 0.05, "sl": 0.02, "min_score": 75, "exit_score": 55},
        {"name": "Balanced", "tp": 0.03, "sl": 0.015, "min_score": 78, "exit_score": 58},
        {"name": "High Profit", "tp": 0.04, "sl": 0.015, "min_score": 80, "exit_score": 60},
        {"name": "Low Risk", "tp": 0.025, "sl": 0.008, "min_score": 85, "exit_score": 65},
        {"name": "Quick Exit", "tp": 0.025, "sl": 0.02, "min_score": 76, "exit_score": 56},
        {"name": "Trend Follow", "tp": 0.035, "sl": 0.012, "min_score": 78, "exit_score": 58},
        {"name": "Volatile", "tp": 0.045, "sl": 0.025, "min_score": 74, "exit_score": 54},
        {"name": "Stable", "tp": 0.015, "sl": 0.01, "min_score": 82, "exit_score": 62},
        {"name": "Optimal", "tp": 0.032, "sl": 0.016, "min_score": 79, "exit_score": 59},
    ]
...
if st.button("▶️ Esegui Simulazione Autonoma + Optimization", key="simulate"):
    ...
    if len(valid_cryptos) >= 12:
        ...
        st.info("🔍 Optimization Loop: testing 10 strategie di Trend Following...")
        with st.spinner("Analisi in corso (1-2 minuti)..."):
            variants_results = test_parameter_variants_trend_following(
        ...
        if variants_results:
            ...
            st.markdown(f"**Strategia Ottimale trovata:**")
            st.markdown(f"• EMA Fast: {int(best_variant['ema_fast'])} | EMA Slow: {int(best_variant['ema_slow'])}")
            
            # Tabella comparativa di tutte le 10 varianti
            st.subheader("📊 Confronto Tutte le Strategie Trend Following")
            display_variants = variants_df[["variant", "ema_fast", "ema_slow", "trades", "win_rate", "profitto", "profitto_pct"]].copy()
...
st.markdown("---")
st.markdown("**ℹ️ OPZIONE C - TREND FOLLOWING PURO (v4):**")
st.markdown("• ❌ **Niente TP/SL rigidi** — strategia vecchia fallita")
st.markdown("• ✅ **Segue il Trend**: entra quando prezzo > EMA50 > EMA200 (UP confermato)")
st.markdown("• ✅ **Esce dinamicamente**: quando prezzo scende sotto EMA50 (trend gira)")
st.markdown("• 🎯 **Testa 10 combinazioni EMA** per trovare la migliore")
st.markdown("• 📊 **Se non funziona**: cambiamo strategia completamente")
