...
st.title("🤖 Crypto Trading Bot - Mean Reversion V11")
st.success("🟢 VERSIONE V11 CARICATA — FILE DI TEST CORRETTO")
st.write("Se stai vedendo questa scritta, l'app sta eseguendo **app crypto bot v11.py**.")
st.caption("BUILD V11 · Coinbase Exchange · 120 giorni · 1 ora")

# COMANDO PRINCIPALE V11 — deve essere visibile subito sotto il marker.
if "start_v11" not in st.session_state:
    st.session_state["start_v11"] = False

def _start_v11():
    st.session_state["start_v11"] = True

st.button(
    "🚀 AVVIA SIMULAZIONE V11",
    type="primary",
    use_container_width=True,
    on_click=_start_v11,
    key="start_v11_button",
)

run_simulation = st.session_state["start_v11"]
st.caption("⬆️ Questo è il pulsante della V11. Se non compare, il deploy NON sta eseguendo questo file.")

st.markdown("Strategia V11: mean reversion con conferma di rimbalzo, RSI e stop/target basati sulla volatilità")
...
def backtest_mean_reversion(data_dict, initial_capital, th1, a1, th2, a2, th3, a3):
    """
    V11 - Mean Reversion con conferma di inversione.
    ...
    variants = [
        {"name": "V11_A", "sma": 50, "dev": .030, "rsi": 30, "confirm": .010, "atr_stop": 1.8, "tp": .030, "max_hours": 48, "cooldown": 12},
        {"name": "V11_B", "sma": 60, "dev": .030, "rsi": 30, "confirm": .010, "atr_stop": 1.9, "tp": .032, "max_hours": 48, "cooldown": 12},
        {"name": "V11_C", "sma": 70, "dev": .035, "rsi": 31, "confirm": .010, "atr_stop": 1.9, "tp": .032, "max_hours": 60, "cooldown": 16},
        {"name": "V11_D", "sma": 80, "dev": .035, "rsi": 30, "confirm": .012, "atr_stop": 2.0, "tp": .035, "max_hours": 60, "cooldown": 16},
        {"name": "V11_E", "sma": 80, "dev": .040, "rsi": 28, "confirm": .012, "atr_stop": 2.0, "tp": .038, "max_hours": 72, "cooldown": 18},
        {"name": "V11_F", "sma": 100, "dev": .040, "rsi": 30, "confirm": .012, "atr_stop": 2.1, "tp": .038, "max_hours": 72, "cooldown": 18},
        {"name": "V11_G", "sma": 60, "dev": .035, "rsi": 28, "confirm": .015, "atr_stop": 2.0, "tp": .035, "max_hours": 72, "cooldown": 18},
        {"name": "V11_H", "sma": 90, "dev": .040, "rsi": 30, "confirm": .015, "atr_stop": 2.1, "tp": .040, "max_hours": 72, "cooldown": 24},
        {"name": "V11_I", "sma": 70, "dev": .040, "rsi": 29, "confirm": .015, "atr_stop": 2.1, "tp": .040, "max_hours": 72, "cooldown": 24},
        {"name": "V11_J", "sma": 100, "dev": .045, "rsi": 28, "confirm": .015, "atr_stop": 2.2, "tp": .042, "max_hours": 96, "cooldown": 24},
    ]
            ...
            if position is None:
                ...
                for crypto, d in ind.items():
                    ...
                    if (
                        ...
                        score = (
                            discount * 100
                            + max(0, variant["rsi"] - rsi) * 0.08
                            + max(0, rsi - prev2_rsi) * 0.12
                            + max(0, recovery) * 80
                            + max(0, macd - prev_macd) / max(abs(prev_macd), 1e-9) * 0.01
                        )
...
# ===================== AVVIO SIMULAZIONE =====================
if run_simulation:
    st.success("🟢 PULSANTE V11 RICEVUTO — LA SIMULAZIONE È PARTITA")
    ...
    if data_dict:
        ...
        st.subheader("📈 Backtest V11")

        with st.spinner("Calcolo delle 10 varianti V11..."):
        ...
        if results:
            ...
            if not trades_df.empty:
                ...
                st.download_button(
                    "📥 Scarica operazioni V11 CSV",
                    data=trades_df.to_csv(index=False),
                    file_name=f"crypto_bot_v11_trades_{int(days)}d.csv",
                    mime="text/csv",
                )

            st.download_button(
                "📥 Scarica risultati V11 CSV",
                data=result_df.to_csv(index=False),
                file_name=f"crypto_bot_v11_results_{int(days)}d.csv",
                mime="text/csv",
            )
...
st.markdown("---")
st.markdown("**Mean Reversion V11 — ingresso solo dopo conferma di inversione**")
