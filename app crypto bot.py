...
st.title("🤖 Crypto Trading Bot - Mean Reversion V10")
st.success("🟢 VERSIONE V10 CARICATA — FILE DI TEST CORRETTO")
st.write("Se stai vedendo questa scritta, l'app sta eseguendo **app crypto bot v10.py**.")
st.caption("BUILD V10 · Coinbase Exchange · 120 giorni · 1 ora")

# COMANDO PRINCIPALE V10 — deve essere visibile subito sotto il marker.
run_simulation = st.button("🚀 AVVIA SIMULAZIONE V10", type="primary", use_container_width=True)
st.caption("⬆️ Questo è il pulsante della V10. Se non compare, il deploy NON sta eseguendo questo file.")

st.markdown("Strategia V10: mean reversion con conferma di rimbalzo, RSI e stop/target basati sulla volatilità")
...
def backtest_mean_reversion(data_dict, initial_capital, th1, a1, th2, a2, th3, a3):
    """
    V10 - Mean Reversion con conferma di inversione.
    ...
    variants = [
        {"name": "V10_A", "sma": 50, "dev": .030, "rsi": 30, "confirm": .010, "atr_stop": 1.8, "tp": .030, "max_hours": 48, "cooldown": 12},
        {"name": "V10_B", "sma": 60, "dev": .030, "rsi": 30, "confirm": .010, "atr_stop": 1.9, "tp": .032, "max_hours": 48, "cooldown": 12},
        {"name": "V10_C", "sma": 70, "dev": .035, "rsi": 31, "confirm": .010, "atr_stop": 1.9, "tp": .032, "max_hours": 60, "cooldown": 16},
        {"name": "V10_D", "sma": 80, "dev": .035, "rsi": 30, "confirm": .012, "atr_stop": 2.0, "tp": .035, "max_hours": 60, "cooldown": 16},
        {"name": "V10_E", "sma": 80, "dev": .040, "rsi": 28, "confirm": .012, "atr_stop": 2.0, "tp": .038, "max_hours": 72, "cooldown": 18},
        {"name": "V10_F", "sma": 100, "dev": .040, "rsi": 30, "confirm": .012, "atr_stop": 2.1, "tp": .038, "max_hours": 72, "cooldown": 18},
        {"name": "V10_G", "sma": 60, "dev": .035, "rsi": 28, "confirm": .015, "atr_stop": 2.0, "tp": .035, "max_hours": 72, "cooldown": 18},
        {"name": "V10_H", "sma": 90, "dev": .040, "rsi": 30, "confirm": .015, "atr_stop": 2.1, "tp": .040, "max_hours": 72, "cooldown": 24},
        {"name": "V10_I", "sma": 70, "dev": .040, "rsi": 29, "confirm": .015, "atr_stop": 2.1, "tp": .040, "max_hours": 72, "cooldown": 24},
        {"name": "V10_J", "sma": 100, "dev": .045, "rsi": 28, "confirm": .015, "atr_stop": 2.2, "tp": .042, "max_hours": 96, "cooldown": 24},
    ]
...
st.markdown("---")
st.markdown("**Mean Reversion V10 — ingresso solo dopo conferma di inversione**")
